...
def slack_post(token: str, channel: str, text: str, thread_ts: Optional[str] = None) -> dict:
    payload = {
        "channel": channel,
        "text": text,
        "unfurl_links": False,
...
def main() -> None:
    ...
    if len(sys.argv) != 2:
        raise RuntimeError("Usage: send_to_slack.py ISSUE_JSON")

    with open(sys.argv[1], encoding="utf-8") as file:
        issue = json.load(file)
    title = (issue.get("title") or "ChatGPT Scheduled Task").strip()
    chunks = split_text(issue.get("body") or "")

    first = f"*{title}*"
    if chunks[0]:
        first += f"\n\n{chunks[0]}"
    result = slack_post(token, channel, first)
    root_ts = result.get("ts")
    for chunk in chunks[1:]:
        slack_post(token, channel, chunk, thread_ts=root_ts)

    print(f"Successfully sent issue #{issue.get('number')} in {len(chunks)} part(s).")
